...
resource "aws_launch_template" "web" {
  ...
  # Each instance exposes / and /health so the ALB can route and check health.
  user_data = base64encode(<<-USERDATA
    ...
    <h1>Region: ${var.region} | AZ: $AZ | Environment: ${var.environment}</h1>
    EOF
    echo "200 OK" > /var/www/html/health
  USERDATA
  )
...
resource "aws_autoscaling_group" "web" {
  ...
  # ELB health checks stop the ASG from keeping instances that fail the ALB target checks.
  launch_template {
