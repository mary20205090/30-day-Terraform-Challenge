...
# Primary region owns the first full stack and the primary Multi-AZ database.
module "vpc_primary" {
  source    = "../../modules/vpc"
...
# The DB trusts only the app-tier security group, not the ASG name.
module "rds_primary" {
  source    = "../../modules/rds"
  providers = { aws = aws.primary }
...
module "rds_replica" {
  ...
  # Required by the variable schema, but ignored for replicas.
  db_name     = ""
  db_username = ""
  db_password = ""
}
